...
export async function GET() {
  try {
    ...
    // Format recent enquiries for UI
    const formattedRecent = recentEnquiries.map((item) => ({
      id: item.id,
    ...
    // Active tours and destinations currently defined in tripConfig catalog
    // (Will migrate to dynamic DB records once Tours/Destinations CRUD tables are created)
    ...
    return NextResponse.json({
      ...
      stats: {
        totalEnquiries,
        totalBookings: confirmedBookings > 0 ? confirmedBookings : totalEnquiries, // total active customer leads/reservations
        confirmedBookings,
        newEnquiries,
        contactedEnquiries,
