...
export function validateAdminCredentials(
  ...
  const validEmail = (process.env.ADMIN_EMAIL || "admin@bharatvistatours.com").trim().toLowerCase();
  const validPassword = (process.env.ADMIN_PASSWORD || "Bharatvista@1810").trim();

  const inputIdentifier = identifier.trim().toLowerCase();
  const inputSecret = secret.trim();

  // Allow full email or username part before @
